...
def analyze_backbone_infrastructure():
    ...
    multi_product_file = Path(export_dir) / "🏗️ Multi-Product Operating Architecture 1e585df01907804c85b6debb645d1f39.html"
    ...
    shared_services_file = Path(export_dir) / "🧩 Shared Services Playbooks 1e685df01907802ab0acefa79d68e227.html"
    if shared_services_file.exists():
        ...
        for block in blocks:
            if block['type'] in ['h2', 'h3']:
                print(f"\n{block['text']}")
            elif block['type'] == 'li' and any(word in block['text'] for word in ['Service', 'Team', 'Platform', 'Infrastructure']):
                print(f"   • {block['text'][:300]}")
    
    # 3. AI Core System
    ai_core_file = Path(export_dir) / "📌 CareAI Core System 1e785df0190780b68806d7a18c14f3b2.html"
    ...
    directory_file = Path(export_dir) / "🤝 Shared Services Directory 1e685df0190780178224c73fd564a794.html"
    ...
    backbone = {
        "Layer 1: Foundation Infrastructure": [
            ...
            "  - Tenant isolation (by clinic, trial, organization)",
        ...
        "Layer 2: Shared Services Platform": [
            ...
            "  - Notification routing by product/tenant",
        ...
        "Layer 4: Platform Services": [
            ...
            "  - Rate limiting per tenant/product",
    ...
    phases = [
        ...
        },
        {
            ...
            "deliverables": [
                "✅ Multi-tenant core with clinic/trial isolation",
    ...
    acceleration = {
        ...
        "ClinicConnect SaaS": [
            "✅ Inherit: Multi-tenant, RBAC, data governance, integration hub, reporting",
