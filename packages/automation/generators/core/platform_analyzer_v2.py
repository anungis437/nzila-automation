...
class TechStack:
    ...
    platform_type: Optional[str] = None  # custom, hybrid, legacy-base44
...
class PlatformAnalyzerV2:
    ...
    def analyze_platform(self, platform_path: Path) -> PlatformProfile:
        ...
        with LogOperation(logger, "analyze_platform", platform=platform_id):
            ...
            profile.database = self._analyze_database_deep(
                platform_path, profile.tech_stack
            )
            ...
            profile.components_count = self._count_components_deep(
                platform_path, profile.tech_stack
            )
            profile.pages_count = self._count_pages(platform_path, profile.tech_stack)
            profile.api_routes_count = self._count_api_routes_deep(
                platform_path, profile.tech_stack
            )
            ...
            profile.migration_estimate_weeks = self._estimate_migration_time_calibrated(
                profile
            )
        ...
        if readme.exists():
            try:
                ...
                if "base44" in content or "low-code" in content:
                    stack.platform_type = "legacy-base44"
                    stack.framework = "NzilaOS (ex-Base44)"
                    stack.language = "TypeScript"
                    logger.info(
                        f"Detected legacy Base44 platform (now NzilaOS) at {path.name}"
                    )
            except Exception as e:
            ...
            try:
                with open(package_json, encoding="utf-8") as f:
                    ...
                    if "next" in deps:
                        ...
                        if stack.platform_type != "legacy-base44":
            ...
            if requirements.exists():
                try:
                    with open(requirements, encoding="utf-8") as f:
                        for line in f:
                            if line.startswith("django==") or line.startswith(
                                "Django=="
                            ):
                    ...
                    logger.warning(
                        f"Error reading requirements.txt for {path.name}: {e}"
                    )
        ...
        if any(path.glob("**/app.py")) or any(path.glob("**/application.py")):
            ...
            if requirements.exists():
                try:
                    ...
                    logger.warning(
                        f"Error reading requirements for Flask detection: {e}"
                    )
    ...
    def _analyze_database_deep(self, path: Path, stack: TechStack) -> DatabaseInfo:
        ...
        with LogOperation(logger, "analyze_database", framework=stack.framework):
            ...
            elif stack.framework in ["Next.js", "React + Vite", "NzilaOS (ex-Base44)"]:
    ...
    def _analyze_django_database(self, path: Path) -> DatabaseInfo:
        ...
        for models_file in path.rglob("models.py"):
            if "migrations" not in str(models_file) and not self._is_ignored(
                models_file
            ):
                try:
        ...
        for migration_file in path.rglob("migrations/*.py"):
            if migration_file.name != "__init__.py" and not self._is_ignored(
                migration_file
            ):
        ...
        logger.debug(
            f"Django: {db.models_count} models, {db.migrations_count} migrations"
        )
        return db
    ...
    def _analyze_nodejs_database(self, path: Path) -> DatabaseInfo:
        ...
        if drizzle_config.exists() or any(path.glob("**/drizzle/**")):
            ...
            for schema_file in schema_files:
                try:
                    ...
                    table_matches = re.findall(
                        r"export\s+const\s+\w+\s*=\s*pgTable\s*\(", content
                    )
                    db.tables_count += len(table_matches)

                    # Count mysqlTable for MySQL
                    mysql_matches = re.findall(
                        r"export\s+const\s+\w+\s*=\s*mysqlTable\s*\(", content
                    )
            ...
            if migrations_dir.exists():
                db.migrations_count = len(
                    [f for f in migrations_dir.glob("*.sql") if not self._is_ignored(f)]
                )

            db.models_count = db.tables_count
            logger.debug(
                f"Drizzle: {db.tables_count} tables from {len(schema_files)} schema files"
            )
        ...
        # Check for Prisma
            ...
            logger.debug(
                f"Prisma: {db.models_count} models, {db.migrations_count} migrations"
            )
        ...
        # Check for Supabase (ENHANCED: better SQL parsing)
                ...
                for migration in migrations_dir.rglob("*.sql"):
                    if not self._is_ignored(migration):
                        ...
                        try:
                            ...
                            table_matches = re.findall(
                                r"CREATE\s+TABLE", content, re.IGNORECASE
                            )
                            db.tables_count += len(table_matches)

                            # Count RLS policies
                            policy_matches = re.findall(
                                r"CREATE\s+POLICY", content, re.IGNORECASE
                            )
                            db.rls_policies += len(policy_matches)
            ...
            db.models_count = db.tables_count
            logger.debug(
                f"Supabase: {db.tables_count} tables, {db.rls_policies} RLS policies"
            )
    ...
    def _count_components_deep(self, path: Path, stack: TechStack) -> int:
        ...
        if stack.framework in [
            "Next.js",
            "React + Vite",
            "NzilaOS (ex-Base44)",
        ] or stack.language in ["TypeScript", "JavaScript"]:
            ...
            if pages_dir.exists():
                for page_file in pages_dir.rglob("*.tsx"):
                    if not self._is_ignored(
                        page_file
                    ) and not page_file.name.startswith("_"):
                        count += 1
        ...
        if package_json.exists():
            try:
                with open(package_json, encoding="utf-8") as f:
                    ...
                    if any(
                        vid_dep in deps
                        for vid_dep in ["remotion", "@remotion/lambda", "video.js"]
                    ):
                    ...
                    # Analytics
                    if any(
                        analytics_dep in deps
                        for analytics_dep in [
                            "@vercel/analytics",
                            "@sentry/nextjs",
                            "sentry",
                        ]
    ...
    def _assess_production_readiness(self, profile: PlatformProfile) -> float:
        ...
        # Testing presence
        if any(
            "test" in dep or "vitest" in dep or "playwright" in dep
    ...
    def _calculate_complexity_calibrated(self, profile: PlatformProfile) -> str:
        ...
        if benchmark:
            logger.info(
                f"Using benchmark complexity for {profile.platform_id}: {benchmark['complexity']}"
            )
    ...
    def _estimate_migration_time_calibrated(self, profile: PlatformProfile) -> int:
        ...
        if benchmark:
            logger.info(
                f"Using benchmark weeks for {profile.platform_id}: {benchmark['weeks']}"
            )
    ...
    def analyze_all(self) -> List[PlatformProfile]:
        ...
        platform_dirs = [
            d
            for d in self.legacy_root.iterdir()
        ...
        for platform_dir in platform_dirs:
            try:
                ...
                logger.error(
                    f"Failed to analyze {platform_dir.name}: {e}", exc_info=True
                )
                print(f"✗ Failed to analyze {platform_dir.name}: {e}")

        self.profiles = [
            asdict(p) for p in profiles
        ]  # Store as dicts for compatibility
        return profiles
...
def main():
    ...
    output_path = (
        Path(sys.argv[2]) if len(sys.argv) > 2 else Path("platform_profiles_v2.json")
    )
    ...
    if profiles:
        ...
        print(
            f"Total Migration Weeks (Sequential): {sum(p.migration_estimate_weeks for p in profiles)}"
        )
        ...
        sorted_profiles = sorted(profiles, key=lambda p: p.entity_count, reverse=True)[
            :5
        ]
