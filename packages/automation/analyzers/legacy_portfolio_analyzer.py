...
class LegacyPortfolioAnalyzer:
        ...
        for item in self.legacy_dir.iterdir():
            if item.is_dir() and not item.name.startswith('.'):
                ...
                if nested_path.exists() and nested_path.is_dir():
                    ...
                    print(f"  → Detected nested folder structure, using: {actual_path.name}")
                ...
                print(f"✓ Found: {item.name} ({self._get_dir_size(actual_path) / (1024 * 1024):.2f} MB)")
    ...
    def _detect_tech_stack(self, path: Path) -> Dict[str, bool]:
        ...
        stack['is_typescript'] = len(list(path.glob('**/*.ts'))) > 0 or len(list(path.glob('**/*.tsx'))) > 0
        stack['is_javascript'] = len(list(path.glob('**/*.js'))) > 0 or len(list(path.glob('**/*.jsx'))) > 0
        
        # Check for Docker
        stack['has_docker'] = (path / 'Dockerfile').exists() or (path / 'docker-compose.yml').exists()
        
        # Check for tests
        test_patterns = ['**/*test*.py', '**/*spec*.js', '**/*test*.js', '**/*spec*.ts', '**/*test*.ts']
    ...
    def _find_package_files(self, path: Path) -> List[str]:
        ...
        package_files = []
        candidates = ['package.json', 'requirements.txt', 'Pipfile', 'pyproject.toml', 'pom.xml', 'build.gradle']
    ...
    def _extract_api_endpoints(self, path: Path) -> List[Dict[str, Any]]:
        ...
        # Express/API routes
        for js_file in list(path.glob('**/routes/**/*.js')) + list(path.glob('**/api/**/*.js')):
            ...
            try:
                with open(requirements, 'r', encoding='utf-8') as f:
                    deps['python'] = [line.split('==')[0].split('>=')[0].strip() 
                                     for line in f if line.strip() and not line.startswith('#')][:30]
            except:
                pass
    ...
    def _classify_vertical(self, platform_name: str) -> str:
        ...
        # Healthtech patterns
        if any(keyword in name_lower for keyword in ['memora', 'clinic', 'health', 'medical']):
            return 'Healthtech'
        
        # Agrotech patterns (check before arts to catch 'cora' correctly)
        if any(keyword in name_lower for keyword in ['farm', 'agro', 'agriculture', 'crop', 'cora']):
            return 'Agrotech'
        
        # Arts & Culture patterns
        if any(keyword in name_lower for keyword in ['congo', 'wave', 'art', 'culture']):
            return 'Arts & Culture'
        ...
        # Uniontech patterns
        if any(keyword in name_lower for keyword in ['union', 'worker', 'labor', 'eyes', 'c3uo']):
            return 'Uniontech'
        
        # Insurancetech patterns
        if any(keyword in name_lower for keyword in ['insurance', 'sentry', 'sentryiq', 'policy', 'claim']):
        ...
        if any(keyword in name_lower for keyword in ['abr', 'racism', 'equity', 'insights']):
            return 'Justice & Equity'
        
        # Trade/Commerce patterns
        if any(keyword in name_lower for keyword in ['trade', 'shop', 'quoter', 'commerce', 'export']):
        ...
        if package_json.exists():
            try:
                with open(package_json, 'r', encoding='utf-8') as f:
                    ...
                    if '@radix-ui/react-dialog' in deps or any('@radix-ui' in d for d in deps):
                        return 'Radix UI'
    ...
    def detect_cross_platform_patterns(self) -> Dict[str, Any]:
        ...
        common_entities = [entity for entity, count in entity_counts.items() if count >= 3]
    ...
    def generate_migration_manifest(self, platform_analysis: Dict[str, Any]) -> Dict[str, Any]:
        """Generate migration manifest for a platform"""
        manifest = {
            "legacy_platform": platform_analysis['name'],
            "target_vertical": platform_analysis['vertical'].lower().replace(' & ', '_').replace(' ', '_'),
            "technology": {
            ...
            "api_endpoints": [ep.get('path', '') for ep in platform_analysis['api_endpoints'][:20]],
            ...
            "migration_strategy": {
                ...
                "backbone_dependencies": self._identify_backbone_deps(platform_analysis)
            },
            "backbone_mapping": {
                "uses_multi_tenant": True,
    ...
    def _identify_backbone_deps(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        deps = ['tenants', 'compliance', 'notifications', 'analytics']
    ...
    def _needs_ai_core(self, analysis: Dict[str, Any]) -> bool:
        ...
        ai_keywords = ['ai', 'ml', 'intelligence', 'cognitive', 'prediction', 'analysis']
    ...
    def _needs_billing(self, analysis: Dict[str, Any]) -> bool:
        ...
        billing_keywords = ['subscription', 'payment', 'billing', 'invoice', 'shop', 'quoter', 'trade']
        name_lower = analysis['name'].lower()
    ...
    def run_full_analysis(self):
        ...
        for platform_name, analysis in self.portfolio_analysis['platforms'].items():
            ...
            print(f"✓ Generated manifest for {platform_name} (Priority: {manifest['migration_strategy']['priority']}, Effort: {manifest['migration_strategy']['estimated_weeks']} weeks)")
        
        self.portfolio_analysis['migration_manifests'] = manifests
        
        # Step 5: Calculate portfolio metrics
        self.portfolio_analysis['portfolio_metrics'] = self._calculate_portfolio_metrics()
        
        return self.portfolio_analysis
    
    def _calculate_portfolio_metrics(self) -> Dict[str, Any]:
        """Calculate overall portfolio metrics"""
        total_entities = sum(len(p['entities']) for p in self.portfolio_analysis['platforms'].values())
        total_components = sum(len(p['components']) for p in self.portfolio_analysis['platforms'].values())
        total_size_mb = sum(p['size_mb'] for p in self.portfolio_analysis['platforms'].values())
        total_migration_weeks = sum(m['migration_strategy']['estimated_weeks'] 
                                   for m in self.portfolio_analysis['migration_manifests'].values())
        
        # Calculate by vertical
        vertical_metrics = defaultdict(lambda: {'platforms': 0, 'entities': 0, 'weeks': 0})
        for platform_name, analysis in self.portfolio_analysis['platforms'].items():
            ...
            vertical_metrics[vertical]['weeks'] += manifest['migration_strategy']['estimated_weeks']
        ...
        return {
            ...
            'roi_without_backbone_weeks': len(self.portfolio_analysis['platforms']) * 24,  # 24 weeks per platform standalone
            'roi_percentage_saved': round((1 - (total_migration_weeks + 16) / (len(self.portfolio_analysis['platforms']) * 24)) * 100, 1)
        }
    ...
    def save_results(self, output_file: str = 'legacy_portfolio_analysis.json'):
        ...
        serializable_analysis = json.loads(
            json.dumps(self.portfolio_analysis, default=lambda x: list(x) if isinstance(x, set) else str(x))
        )
    ...
    def print_summary_report(self):
        ...
        print(f"\n💰 ROI CALCULATION:")
        print(f"  Without Backbone: {metrics['roi_without_backbone_weeks']} weeks ({metrics['total_platforms']} platforms × 24 weeks)")
        print(f"  With Backbone: {metrics['total_with_backbone']} weeks (16 backbone + {metrics['total_migration_weeks']} migrations)")
        print(f"  Time Saved: {metrics['roi_without_backbone_weeks'] - metrics['total_with_backbone']} weeks ({metrics['roi_percentage_saved']}% faster)")
