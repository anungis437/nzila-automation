...
| Layer | Technology |
|---|---|
| Platform | **Base44** (low-code PaaS) — app scaffold, backend, auth, entities/DB all managed by platform |
| Frontend | React 18 + Vite 6 + React Router 6 (SPA) |
| State/Data | TanStack React Query v5 — all pages fetch via `base44.entities.*.list()` |
| UI | Radix UI + Tailwind CSS 3 + shadcn/ui pattern |
| Serverless | **Deno** (`Deno.serve`) — 44 TypeScript cloud functions via `@base44/sdk@0.8.6` |
...
- Fully delegated to Base44 SDK: `base44.auth.me()`, `base44.auth.logout()`, `base44.auth.redirectToLogin()`
...
- MFA functions exist but are custom implementations on top of Base44
...
All entities defined in Base44 backend (no local schema files). Inferred from usage:
...
| Integration | Usage |
|---|---|
| Base44 Core SDK | Entity CRUD, file upload, email, LLM invocation |
| Base44 LLM (AI) | OMR processing uses `InvokeLLM` with JSON schema for OCR |
...
| `base44.entities.*` called directly in components — vendor lock-in | **HIGH** |
...
| Asset | Reason |
|---|---|
| Base44 SDK dependency | Proprietary PaaS lock-in |
...
| Base44 pages.config.js scaffold | Replace with NzilaOS routing |
