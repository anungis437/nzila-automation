---
title: Welcome to Nzila
description: An introduction to the Nzila digital-venture studio and our platform ecosystem.
category: Getting Started
order: 1
date: 2026-02-01
---

## What is Nzila?

Nzila is a digital-venture studio building a portfolio of interconnected platforms across multiple verticals — from legal-tech and fintech to cybersecurity education and cultural media.

We operate at the intersection of **AI, social impact, and enterprise software**, building tools that create real economic participation for communities that have historically been underserved by technology.

> "Nzila" means *path* or *way* in several Bantu languages. It reflects our mission: building the infrastructure pathways that connect communities to economic opportunity.

---

## Our Approach

We operate as a **multi-vertical platform company**, sharing a common technology backbone across all products:

- **Shared infrastructure** — Authentication, billing, analytics, and deployment pipelines are built once and reused.
- **Vertical specialisation** — Each product targets a specific market with deep domain expertise.
- **Data-driven iteration** — Every platform feeds insights back into a central analytics engine.
- **Security by default** — Org isolation, audit trails, and evidence integrity are platform primitives — not afterthoughts.

---

## Our Platforms

| Platform | Vertical | Stage |
|----------|----------|-------|
| **Union Eyes** | Labour / Fintech | Live |
| **ABR Insights** | Compliance / Legal-tech | Live |
| **Cora** | AI Automation | Beta |
| And 12 more | Various | Roadmap |

---

## How This Site Is Organised

This resources section contains public documentation aimed at three audiences:

1. **Developers** — technical guides, API references, and integration docs
2. **Partners** — integration specs, white-label docs, and onboarding materials
3. **Stakeholders** — architecture overviews and strategic context

Use the category filters above to find what you need, or browse below to start exploring.

---

## Explore Further

- [Platform Architecture](/resources/platform-architecture) — How our Backbone connects every vertical.
- [Security Overview](/resources/security-overview) — How we protect your data.
- [Developer Guide](/resources/developer-guide) — Build on the Nzila platform.
- [Partner Integration](/resources/partner-integration) — Connect your product to ours.
- [Products](/products) — See our flagship platforms in action.
- [Contact](/contact) — Get in touch with the team.
