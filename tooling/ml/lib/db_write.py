...
def start_training_run(org_id: str, model_key: str, dataset_id: str | None) -> str:
    ...
    with _conn() as conn, conn.cursor() as cur:
        cur.execute(
            ...
            INSERT INTO ml_training_runs
              (id, org_id, model_key, dataset_id, status, started_at, created_at, updated_at)
            VALUES (%s, %s, %s, %s, 'started', %s, %s, %s)
            """,
            (run_id, org_id, model_key, dataset_id, _now(), _now(), _now()),
        )
    ...
    with _conn() as conn, conn.cursor() as cur:
        cur.execute(
            ...
            INSERT INTO ml_models
              (id, org_id, model_key, algorithm, version, status,
            ...
            ON CONFLICT (org_id, model_key, version) DO NOTHING
            """,
            (
                model_id, org_id, model_key, algorithm, version,
                training_dataset_id, artifact_document_id, metrics_document_id,
    ...
    with _conn() as conn, conn.cursor() as cur:
        cur.execute(
            ...
            INSERT INTO ml_inference_runs
              (id, org_id, model_id, status, started_at,
               input_period_start, input_period_end,
               summary_json, created_at, updated_at)
            VALUES (%s, %s, %s, 'started', %s, %s, %s, '{}', %s, %s)
            """,
            (run_id, org_id, model_id, _now(), period_start, period_end, _now(), _now()),
        )
    ...
    with _conn() as conn, conn.cursor() as cur:
        cur.execute(
            ...
            INSERT INTO ml_scores_stripe_daily
              (id, org_id, date, features_json, score, is_anomaly,
               threshold, model_id, inference_run_id, created_at)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            ON CONFLICT (org_id, date, model_id)
            DO UPDATE SET
    ...
    with _conn() as conn, conn.cursor() as cur:
        cur.execute(
            ...
            INSERT INTO ml_scores_stripe_txn
              (id, org_id, stripe_event_id, stripe_charge_id,
    ...
    with _conn() as conn, conn.cursor() as cur:
        cur.execute(
            ...
            INSERT INTO documents
              (id, org_id, category, title, blob_container, blob_path,
...
def upsert_ue_priority_score(
    *,
    ...
    Unique constraint: (org_id, case_id, model_id).
    """
    with _conn() as conn, conn.cursor() as cur:
        cur.execute(
            """
            INSERT INTO ml_scores_ue_cases_priority
              (id, org_id, case_id, occurred_at, score, predicted_priority,
               actual_priority, features_json, model_id, inference_run_id, created_at)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            ON CONFLICT (org_id, case_id, model_id)
            DO UPDATE SET
    ...
    with _conn() as conn, conn.cursor() as cur:
        cur.execute(
            ...
            INSERT INTO ml_scores_ue_sla_risk
              (id, org_id, case_id, occurred_at, probability, predicted_breach,
    ...
    with _conn() as conn, conn.cursor() as cur:
        cur.execute(
            ...
            INSERT INTO audit_events
              (id, org_id, actor_clerk_user_id, action,
               target_type, target_id, after_json, hash, created_at)
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)
            """,
            (
                str(uuid.uuid4()), org_id, actor, action,
