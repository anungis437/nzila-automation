/**
 * External Communication System Schema
 * 
 * Stores data from communication platforms (Slack, Microsoft Teams, etc.)
 * including channels, messages, users, and shared files.
 * 
 * Provider-agnostic design supports multiple communication platforms.
 */
...
export const externalCommunicationMessages = pgTable(
  'external_communication_messages',
  {
    ...
    externalProvider: varchar('external_provider', { length: 50 }).notNull(),
    externalId: varchar('external_id', { length: 255 }).notNull(),
    channelId: integer('channel_id').references(() => externalCommunicationChannels.id, {
      onDelete: 'cascade',
    }),
