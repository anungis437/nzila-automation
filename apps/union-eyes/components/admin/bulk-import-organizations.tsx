/**
 * Bulk Import Organizations Component
 * 
 * Upload and import multiple organizations from CSV/Excel files.
 * Features:
 ...
 * - Duplicate detection
 * 
 * @module components/admin/bulk-import-organizations
 */
...
export function BulkImportOrganizations({
  open,
  onOpenChange,
  onSuccess,
}: BulkImportOrganizationsProps) {
  ...
  const downloadTemplate = useCallback(() => {
    ...
    link.href = url;
    link.download = "organizations-import-template.csv";
    link.click();
  ...
  // Preview import
  const previewImport = async (fileToPreview: File) => {
    setIsUploading(true);
    setError(null);
    ...
    try {
      ...
      const response = await fetch("/api/admin/organizations/bulk-import", {
        method: "POST",
        body: formData,
      });
  ...
  const executeImport = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Failed to import organizations");
    } finally {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle>Bulk Import Organizations</DialogTitle>
          <DialogDescription>
            Upload a CSV or Excel file to import multiple organizations at once.
          </DialogDescription>
        </DialogHeader>
              ...
              {importResult.success ? (
                <Alert className="border-green-200 bg-green-50 dark:bg-green-950">
                  ...
                  <AlertDescription className="text-green-600 dark:text-green-400">
                    Successfully imported {importResult.created} organization(s)!
                  </AlertDescription>
                </Alert>
              ) : (
                <Alert variant="destructive">
                  ...
                  <AlertDescription>
                    ...
                    {importResult.created > 0 &&
                      ` ${importResult.created} organization(s) were created.`}
                  </AlertDescription>
                </Alert>
              )}
