...
- Org-level data isolation enforcement
...
class OrganizationIsolationMiddleware(MiddlewareMixin):
    ...
    1. All database queries are scoped to the request's organization (Org)
    2. Cross-org data access is blocked at the request layer
    3. Org context is available in views via request.organization
    """

    def process_request(self, request):
        """Attach organization object to request for org-scoped DB access."""
