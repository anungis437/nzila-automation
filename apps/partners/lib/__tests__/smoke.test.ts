/**
 * Partners — Platform Smoke Tests
 *
 * Validates the partners app's platform integration modules.
 */
...
describe('Partners platform integration', () => {
  ...
  it('has Zod env validation', () => {
    expect(
      existsSync(resolve(APP, 'lib/env.ts')) || existsSync(resolve(APP, 'env.ts')),
    ).toBe(true)
  })
})
